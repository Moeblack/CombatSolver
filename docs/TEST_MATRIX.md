...
> 基线：CombatSolver `0.23.0`（当前定版；创意工坊稳定版为 `0.22.9`）、塔 2 `0.111.0`、RitsuLib 实测 `0.5.18`（清单最低 `0.5.13`）、CombatSolver 内置战斗模拟引擎。无人测试运行隔离的原版 `--headless` 游戏进程，不使用自建 STS CLI；性能最终门槛另由 Steam 可见会话验证。完整战斗基准使用 `Instant / 0 秒` 部署。
...
| 场景 | 结果 | 验证内容 | 日期 |
| --- | --- | --- | --- |
| `AEONGLASS-LISTENER-CACHE-FORK` | 通过（headless 语义门禁） | 首次 COW 精确替换子分支 preview listener，后续普通字段写不重建缓存，附魔/苦难增删与移出战斗精确更新 listener；父分支、OwnerPile 与完整根快照隔离同时通过。最终 runId `d0a21ebdfbf2411e8319304371e190bd`。 | 2026-09-01 |
| `BASELIB-CARD-MODIFIER-LISTENER-CACHE-FINAL3` | 通过（完整 BaseLib headless 保守兼容门禁） | 从两张初始未修饰卡验证侧表 modifier 动态增删；`Amount`/`Priority`、fingerprint、choice-key、continuation 和 listener 均随分支精确变化，Owner、父分支与侧表容器保持隔离。首次 live stamp 经根 modifier 注册后保持稳定；空 modifier 生成牌可按同一状态键部署匹配，首次遇到未登记的非空 modifier 时可保守发现并独立克隆。`StoreSaveData` 的 `IntProperties`/`AdditionalProperties` 编码为静态实现审计。从 deck+已移除源卡生成的 clone 独立复制 modifier、重绑 Owner 且 listener 可见，同时保持 `DeckVersion=null / HasBeenRemovedFromState=false`。runId `7ef16ab8cf5b4760a8a27e04ac694e75`。 | 2026-09-01 |
| `GC-LIFECYCLE-POLICY-MECHA-010` | 通过（headless 合成政策门） | 低分配 active scope 的 early No-GC exit 等待旧搜索、阻止新 entrant 与 root-capture barrier，且不强制 Gen2；fault/cancel 的释放任务不污染后续 FIFO。区域外根压力和区域内重分配两条路径的 `295,200,624 B` 弱引用连通图均死亡，各只完成一次非压缩 Gen2；区域内并发请求为 `reclaim=1 / join=1 / Gen2=1`。runId `8441e817afdf402d95567eb4d7d11607`。 | 2026-09-01 |
| `GC-CONTROLLER-RELEASE-MECHA-011` | 通过（headless 控制器生命周期门） | A→B→Reset 将当前与先前已换下的搜索会话、实际主线程 callback、visual-setup 延迟任务以及两个已换下的合成部署 operation 纳入释放屏障。搜索和部署各自均为 `scheduled=2 / completed=2 / CTS disposed=2`；旧延迟任务取消后不执行，旧回合准备 epoch 的 failure/mismatch 不写入新会话，最终断言后的测试结果引用在复用 GC 前清空。runId `57ec7ee8c2634d95ae0c358a336ae7cf`；正常 `Setup → AutoPrePlay → 熵选牌 → 第 2 回合复用` 另由 runId `6f09011103084daaaca36084264eacdb` 通过。 | 2026-09-01 |
| `SEARCH-PERF-IRONCLAD-CLONE-HAVOC-ROOT-A/B` | 通过（清空默认牌组的固定根 A/B） | 同一 `2305` 张战斗牌、`2302` 张永久牌组牌、`4499` listener 的根隔离输入保持 continuation 全字段一致；测试阶段 `4570.556 → 2215.689 ms`，搜索时生产根捕获 `3971.045 → 2235.620 ms`。基线/最终 runId `9e1b791c1f744b8aa91c5939cd8dd9e8` / `228b7a5750fa434db93c93f8997ec23f`。 | 2026-09-01 |
| `SEARCH-PERF-IRONCLAD-CLONE-HAVOC-1S-A/B` | 通过（清空默认牌组的固定工作量 A/B） | 两侧均为 `1/3/3/2` 的 expanded/transitions/forks/replays，同一 `ENTROPY` 路线、评分 `-3752001`、TimeLimit、0 GC；`5186.2 ms / 912,561,760 B` 对 `3471.0 ms / 267,554,848 B`，耗时降低 `33.07%`、分配降低 `70.68%`、managed-live 降低 `16.20%`、工作集降低 `11.98%`。基线/最终 runId `c386a841076f4350aeec50c718ebc49c` / `5e7c3c0126d04cf3ad8a0a2535427cae`。 | 2026-09-01 |
