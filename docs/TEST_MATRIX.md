...
> 基线：CombatSolver `0.21.6`（当前定版；创意工坊稳定版 `0.21.5`）、塔 2 `0.111.0`、RitsuLib 实测 `0.5.18`（清单最低 `0.5.13`）、CombatSolver 内置战斗模拟引擎。无人测试运行隔离的原版 `--headless` 游戏进程，不使用自建 STS CLI；性能最终门槛另由 Steam 可见会话验证。完整战斗基准使用 `Instant / 0 秒` 部署。
...
## 0.21.6 后续（开发中）：搜索热路径分配优化

| 场景 | 结果 | 验证内容 | 日期 |
| --- | --- | --- | --- |
| `SEARCH-PERF-V0213-MECHA-FIXED-WORK-A/B` | 通过（v0.21.3 历史固定工作量 A/B） | `upstream/main@9f815f9` 与当时的优化 artifact 均保持 `4319` 展开、`33087` 转移、`18399` 选牌分支及相同第 7 回合/28 战损路线。上游 runId `ac3166a7460c4e0daffe6ca2f87e3e68` 为 `7083.0 ms / 4,480,779,728 B`，优化 runId `5fdd123098a2494cbe291814ce83f4c8` 为 `6543.9 ms / 2,768,318,896 B`；累计分配降低 `38.2%`，单次 headless 墙钟缩短 `7.6%`，后者不替代可见 Steam 结论 | 2026-08-30 |
| `SEARCH-PERF-V0213-LARGE-DECK-FIXED576-A/B` | 通过（v0.21.3 历史固定工作量 A/B） | 匿名大牌组压力根在上游与当时的优化 artifact 均固定到 `576` 展开、`1124` 选牌分支，并返回相同 3 回合、预计掉血 `2` 的策略结果。上游 runId `2a0d0dcc058e49ddb8142ce58e4ea9ac` 为 `5116.3 ms / 1,039,502,640 B`，优化 runId `70958583096e45b8b47f974f4cdbcfd8` 为 `3985.0 ms / 528,876,328 B`；累计分配降低 `49.12%`，单次 headless 墙钟缩短 `22.11%` | 2026-08-30 |
| `SEARCH-PERF-V0216-FULL-DEEP-16GB` | 通过（当前 artifact，headless 精确设置完整长搜） | runId `0aea27ef6b8d4474afcf2e1419b5877a` 精确进入 `16,000,000,000 B` No-GC 区域，保持原 `16/120s`、Beam `18/45`、节点 `2400/12000`、分支、DOP8 和药水策略；`53605.8 ms / 11,655,259,632 B` 后自然穷尽，仍为 `7018/52644/23196` 工作量、评分 `9998081943`、第 10 回合胜利、玩家/敌人 HP `9/0`、预计战损 `56`、卖血 `11` 并使用 2 瓶药。相比 v0.21.5 同工作量结果，墙钟 `+1.12%`、累计分配 `+0.077%`，按单次 headless 波动处理；GC0/1/2=`2/2/2`、总/最大暂停 `13.0/11.8 ms` | 2026-08-30 |
| `SEARCH-PERF-V0213-LARGE-DECK-PROFILE` | 已完成（历史诊断证据） | 匿名旧问题进程记录至少 `32.835 GB` allocation delta、`16.27 GB` 工作集峰值和 `37.09 GB` private 峰值；搜索后 GC dump 仅 `92 MB / 816539` 对象，约 `92.8%` 的搜索期占用在 GC 后消失，结论为瞬时 churn 而非 retained leak。fixed576 trace 显示池化身份哈希已移除原线性 Find 热点，剩余集中在 `PredictedCard`、牌堆、列表容量与 `ToArray` | 2026-08-30 |
| `SEARCH-PERF-V0214-POTION-HEAVY-PARTIAL` | 超时，不计完整结果 | 匿名小牌组战前重建以 VeryHigh、DOP8、`RequireAtLeastOne` 和精确 `16 GB` 运行；runId `a90cd48eab8a4b6885dcbfb1303b2549` 在主 Deep 完成 `14714` 展开、`248003` 转移、`74.407 s / 21.020 GB` 后，于强制用药审计阶段达到固定 `120 s` 外层时限。action/potion-exec 分配桶约 `11.609/7.122 GB`，Fork/snapshot/fingerprint 约 `4.743/2.468/1.462 GB`。该重建是压力样例，不是严格 replay 或旧新路线 A/B | 2026-08-30 |
| `SEARCH-PERF-V0214-PRESET-CONTRACTS` | 通过（No-GC 功能实现门禁） | Low/Medium/High/VeryHigh 分别断言原 `5/60`、`8/120`、`12/180`、`20/300` 秒及原 Beam、节点、分支预算，并实际进入精确 `6/8/12/16 GB` 区域。runId `ad7b0fc19ac449eeb7c6a89b7237bbb0`、`652255f684f3437c8b8d5f9e05de7848`、`5ee22dc39e2248f192bfc3b2e4a04b9d`、`3ad60c5ca2a04bf6a2cfb07df7cb3d4d` | 2026-08-30 |
| `SEARCH-PERF-V0216-NOGC-RNG-BUDGET-CONTRACT` | 通过（当前 artifact） | SearchPolicy 门禁实际进入 `1 GB` 区域，令并发 `2 GB` 请求等待，取消并释放旧 scope 后精确重建 `2 GB`；同时验证完整 RNG 身份及 DOP1/DOP2 结果全字段一致，runId `5677b8ccffc842d68f2199da964ed610` | 2026-08-30 |
| `SEARCH-PERF-V0216-SANITIZED-FIXTURES` | 通过（当前 artifact，仓库内脱敏短测） | Silent 396 张合成牌堆 runId `a2aef73ea38345a7b48418f7ff498ffc` 为 `3990.4 ms / 1,466,872,064 B`、`2218/8823/3236`；Necrobinder 最小战前投影使用公开合成 seed，runId `07427794ff05455886fc7faf2318966e` 为 `5051.1 ms / 2,219,543,784 B`、`1949/27872/18919`，并使用 1 瓶药。两项都断言 VeryHigh 原 Beam `36/90`、节点 `10000/50000`、分支 `30/48` 和 No-GC `16 GB` | 2026-08-30 |
| `SEARCH-PERF-V0216-POTION-CHOICE-ALLOCATION` | 通过（当前 artifact 严格语义门禁） | 生成牌药水只克隆实际选择，选牌 occurrence 查找移除 LINQ；候选、token、顺序、分支上限、药水政策和评分未改。17 项药水完整原版/预测差分 runId `341bf965156644c4a8fa6e3cd2399682` 为 17/17，赌徒特酿专项 runId `c5f1b95001f542d3b0d536295f914bdc` 通过 | 2026-08-30 |
| `SEARCH-PERF-V0213-WIDER-WAVE` | 未采用 | 更宽 wave 会同时保活更多 `ExpansionBatch` 与 raw snapshot，与本轮降低峰值内存的目标冲突。下一步先补 raw snapshot/工作集峰值观测，再对 `seed + steal overflow` 做固定工作量 A/B，不把扩大 wave 当作已完成优化 | 2026-08-30 |
| `SEARCH-PERF-CACHE-FORK-BOUNDARIES` | 通过（当前 artifact） | Ritsu capability 缓存、选牌语义键跨 Fork、池化身份哈希、listener observer、所属牌堆 O(1) 映射、投影洗牌、稀疏 Power affliction 和 `CardPlay` 选择风险隔离由 Fork/CombatRootSnapshot 门禁覆盖。runId `bd24e4eeda0247f99eaac9fa90281e3b` | 2026-08-30 |
| `SEARCH-PERF-POLICY-EQUIVALENCE` | 通过（当前 artifact） | 内建 250 节点门禁递归比较 DOP1/DOP2 的动作与嵌套选择、评分、展开、转移、全部非时序剪枝、快照、continuation、完整 RNG 身份和回合标注一致；DOP2 实际产生并发，两档都释放节点上限丢弃快照。当前 SearchPolicySnapshot/CombatRootSnapshot runId `5677b8ccffc842d68f2199da964ed610` | 2026-08-30 |
| `SEARCH-PERF-FULL-AUTO-DOP2` | 通过（`dbcc4b4` 中间态历史） | 显式 DOP2 完整自动部署双小啮兽：第 5 回合结束、两次洗牌、零战损、零药、计划外重算 `0`。runId `43a38cee74d04a42b48b1360c409ab39`；该行不冒充最终 `9f815f9` artifact 的重复实跑 | 2026-08-30 |
| `SEARCH-PERF-INCREMENTAL-EQUIVALENCE` | 通过（v0.21.3 历史 artifact） | 战斗根隔离与逐转移增量/完整前缀回放等价通过；双小啮兽第 5 回合结束、两次洗牌、零战损/零药且零计划外重算。runId `869cb7c5967e4e6ebf09aed7222f4b5b` | 2026-08-30 |
| `SEARCH-PERF-TARGET-QUALITY` | 通过（`dbcc4b4` 中间态历史） | 高预设贪食者固定根保持本回合掉血 `0`、预计全战斗掉血 `7`、零卖血/零药、搜索至少 7 回合且最终敌方生命不高于 `204`。runId `828f39da032842b5ba460a07c342363e`；最终 artifact 的质量不降证据以前述 fixed-work 同路线 A/B 为准 | 2026-08-30 |
| `SEARCH-PERF-VISIBLE-STEAM` | 外部阻断，不计通过 | 完整栈因 Ave Mujica 的不支持 `DreamspinFtue` subscriber 在搜索前 fail-fast，runId `c02dd20a406e45998b605a3aa4013904`；兼容子集 runId `067cf060be1a475aa9bb1b2a8e239a07` 实际加载创意工坊 `0.21.1`，不算本分支证据；强制本地旧合并态 runId `f79d84fa7aae490398a05ad41685e30e` 暴露了上游现已修复的 BaseLib 重复键。拉取后强制本地 DLL 的 Steam 请求在 `60s` 内未创建进程，随后 `30s` 无延迟进程；7 个临时 manifest 均已恢复。本轮不写正式可见 Steam 速度结论 | 2026-08-30 |
| `SEARCH-PERF-REFACTOR-BOUNDARIES` | 通过 | Release 构建零警告；`verify-refactor-boundaries.sh` 返回 `REFACTOR_BOUNDARIES_OK search_files=54` | 2026-08-30 |
